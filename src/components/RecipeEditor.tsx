...
const commonUnits = ['g', 'kg', 'ml', 'l', 'ks', 'lžíce', 'lžička', 'hrst', 'špetka', 'balení', 'porce'];
...
const RecipeEditor: React.FC<RecipeEditorProps> = ({
  ...
  <select 
    ...
    onChange={(e) => {
      ...
      if (e.target.value.startsWith('RECIPE:')) {
        unitInput.value = 'porce';
      }
      // Vynutíme překreslení pro aktivaci/deaktivaci readOnly (React referencí by to bylo čistší, 
      // ale držíme se vašeho DOM přístupu)
      const forceUpdate = (this as any); 
    }}
  >
  ...
  <input 
    ...
    placeholder="Jedn." 
    list={`units-${idx}`}
    // Pokud select obsahuje RECIPE, pole bude zamčené
    onFocus={(e) => {
      const sel = document.getElementById(`ing-name-select-${idx}`) as HTMLSelectElement;
      if (sel.value.startsWith('RECIPE:')) {
  ...
  <button className="btn insert-btn" onClick={() => {
    ...
    const unitInput = document.getElementById(`ing-unit-select-${idx}`) as any;
    if (!sel.value || !val.value) return alert("Vyberte položku a zadejte množství.");

    const finalUnit = sel.value.startsWith('RECIPE:') ? 'porce' : unitInput.value;
    const n = [...editSteps];
    n[idx] = n[idx] + (n[idx].length > 0 && !n[idx].endsWith(' ') ? ' ' : '') + `{{${sel.value}|${val.value}|${finalUnit}}}`;

    setEditSteps(n);
    val.value = "";
    unitInput.readOnly = false; // Reset pro další vkládání
  }}>VLOŽIT</button>
</div>
            </div>
          ))}
